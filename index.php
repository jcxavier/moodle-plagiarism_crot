 ...
 */
    ...
    if (!$subA = $DB->get_record("crot_files", array("id" => $submA->crot_submission_id)))  {
	...
	if (!$submissionA = $DB->get_record("progassessment_submissions", array("file" => $fileA->id))) {
        print_error(get_string('incorrect_submAid','plagiarism_crot'));
    }
	
    if (!$assignA = $DB->get_record("progassessment", array("id" => $submissionA->progassessment))) {
		print_error(get_string('incorrect_assignmentAid','plagiarism_crot'));
	}
    
    if(!has_capability('mod/progassessment:grade', get_context_instance(CONTEXT_MODULE, $subA->cm))) {
        print_error(get_string('have_to_be_a_teacher', 'plagiarism_crot'));
    }
    // build navigation and header    
    $view_url = new moodle_url('/mod/progassessment/view.php', array('id' => $subA->cm));
    ...
    $sql_query = "SELECT * FROM {$CFG->prefix}crot_submission_pair WHERE submission_a_id ='$ida' OR  submission_b_id = '$ida' order by number_of_same_hashes desc";
    $similars = $DB->get_records_sql($sql_query);
    $sql_query = "SELECT count(*) as cnt from {$CFG->prefix}crot_fingerprint where crot_doc_id = '$ida'";
	...
	if (!empty($similars)){
        foreach ($similars as $asim){
		    ...
		    $subm3 = $DB->get_record("crot_documents", array("id"=>$partner));
            $party = $partner;
		
		    if ($subm3->crot_submission_id == 0) {
			// web document
                $wwwdoc = $DB->get_record("crot_web_documents", array("document_id"=>$party));					
                $nURL = urldecode($wwwdoc->link);
                $namelink = substr($nURL,0,40);
                $courseBname = get_string('webdocument','plagiarism_crot');
		    }
		    else {
				$subm4 = $DB->get_record("crot_files", array("id"=>$subm3->crot_submission_id));
