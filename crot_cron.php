...
$sql_query = "SELECT cf.* FROM {crot_files} cf where cf.status = 'queue'";
$files = $DB->get_records_sql($sql_query);
if (!empty($files)){
    foreach ($files as $afile){
        try {
        $afile->status = 'in_processing';     
        $DB->update_record('crot_files', $afile);
        ...
        // call tokenizer to get plain text and store it in crot_documents
        $atext = tokenizer ($apath, $ext);
        
        $atext = mb_substr($atext, 0, 999000);
      // insert into crot_documents  
      //  $docrecord = new object();
      //  $docrecord->crot_submission_id = $afile->id;
	  //  $docrecord->content = addslashes($atext);
      //  $docid = $DB->insert_record('crot_documents', $docrecord);
        $content = addslashes($atext);
        $query = "INSERT INTO {$CFG->prefix}crot_documents (content, crot_submission_id) VALUES ('$content','$afile->id')";
        mysql_query($query);
		...
		foreach ($fingerp as $fp) {
			...
			$DB->insert_record("crot_fingerprint", $hashrecord);	
		}
        
        // local search
        $plagiarismvalues = $DB->get_records_menu('crot_config', array('cm'=>$afile->cm),'','name,value');
        ...
        if ($plagiarismvalues['crot_local']==1) {
			...
			foreach ($pair_submissions as $pair_submission){
				...
				if ($webdoc = $DB->get_record("crot_web_documents", array('document_id'=>$pair_submission->id)))
					continue;
				...
				if ($docid!=$pair_submission->id){
					...
					if (! $pair){
						...
						$DB->insert_record("crot_submission_pair", $pair_record);	
					} else {
						// TODO update		
					}
        ...
        if ($plagiarismvalues['crot_global']==1) {
			...
			//	$wdocid = $DB->insert_record("crot_documents", $wdocrecord);
                $wdoc_content = addslashes($result);
                $query = "INSERT INTO {$CFG->prefix}crot_documents (content, crot_submission_id) VALUES ('$wdoc_content','0')";
                mysql_query($query);
			...
			/*	$webdocrecord->document_id = $wdocid;
				...
				$webdocid = $DB->insert_record("crot_web_documents", $webdocrecord);*/
                $weblink = urlencode($manUrl->mainUrl);
                $weblinklive = urlencode($manUrl->msUrl);
                $query = "INSERT INTO {$CFG->prefix}crot_web_documents (document_id,link,link_live,is_from_cache,related_doc_id) VALUES ('$wdocid','$weblink','$weblinklive','false','$docid')";			
                mysql_query($query);
				...
				if(!is_null($similarnumber->cnt) && $similarnumber->cnt!=0 ){
					...
					$pair_record->number_of_same_hashes = $similarnumber->cnt;
					$ppair = $DB->insert_record("crot_submission_pair", $pair_record);
				} else {
					//if null then remove the web document and its fingerprint
					// remove from doc
					$DB->delete_records("crot_documents", array("id"=>$wdocid));
					// remove from fingerprints
					$DB->delete_records("crot_fingerprint", array("crot_doc_id"=>$wdocid));
				}					
			}	
...
echo "\nThe assignments were processed by crot in ".$totaltime." seconds\n";
?>
