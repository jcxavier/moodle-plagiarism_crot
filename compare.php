 ...
 */
	...
	if (!$isWebB) {
		if (! $subB = $DB->get_record("crot_files", array("id" => $submB->crot_submission_id))) {
			print_error(get_string('incorrect_fileBid','plagiarism_crot'));
		}
	        if (!$fileb = $DB->get_record("files", array("id" => $subB->file_id))) {
    		    print_error(get_string('incorrect_fileBid','plagiarism_crot'));
	        }
    		if (!$submissionB = $DB->get_record("progassessment_submissions", array("file" => $fileb->id))) {
	            print_error(get_string('incorrect_submBid','plagiarism_crot'));
    		}
		if (! $assignB = $DB->get_record("progassessment", array("id" => $submissionB->progassessment))) {
		...
		require_course_login($courseB);
    		if(!has_capability('mod/progassessment:grade', get_context_instance(CONTEXT_MODULE, $subB->cm))) {
	            print_error(get_string('have_to_be_a_teacher', 'plagiarism_crot'));
    		}
	}
